--[[
  Most of this code is copied from Setup/Rally Exchange. Might be an interesting idea to turn this thing into
  some sort of library?
]]

---@alias StoreEntry { metadata: { name: string, canBeShared: boolean }, export: (fun(s: self, name: string): string) }
---@alias EntryID string
---@type {targetName: string?, list: (fun(): StoreEntry[]), add: (fun(data: string, trackID: string?): EntryID), (delete: fun(id: EntryID)), (select: fun(id: EntryID)), current: (fun(): EntryID)}
local DataProvider

local AppConfig = ac.storage { userName = '', setupsOrder = 1 }
if AppConfig.userName == '' then
  AppConfig.userName = ac.getDriverName(0)
end

local mainTrackID = '_'
local endpoint = 'https://rgb.acstuff.club'
...
local function createSession(callback)
  ac.uniqueMachineKeyAsync(function(err, data)
    ...
    web.request('POST', endpoint .. '/session', { ['X-Session-ID'] = '0' }, JSON.stringify { userID = userID },
...
local function rest(method, url, data, callback, errorHandler)
  ...
  if not callback then
    callback = function(response, headers)
      ac.log('Successfully executed: ' ..
        url .. ', response: ' .. stringify(response))
    end
  end
...
local function refreshGenList(uniqueKey, url, params, callback, continuationState)
  ac.log(url, params)
  rest('GET', url, table.chain(params, { offset = continuationState and #continuationState[3] or 0, limit = limit }),
    function(response, headers)
      local totalCount = tonumber(headers['x-total-count']) or #response
      if callback then
        local continuationState = { #response < totalCount, totalCount, response, table.map(response,
          function(item) return true, item[uniqueKey] end) }
...
local function downloadSetupAsFile(setupInfo)
  ...
  getSetupData(setupInfo, true, function(err, data)
    ...
    local added = DataProvider.add(data, setupInfo.trackID)
    ui.toast(ui.Icons.Confirm, 'Preset loaded', function()
      DataProvider.delete(added)
...
local function refreshComments()
  if not listOfComments then
    local key = math.random()
    listOfComments = key
    refreshGenList('commentID', 'comments', { setupID = discussingItem.setupID },
      function(ret, continuation, totalCount)
...
local function shareSetup(pn, name)
  try(function()
    rest('POST', 'setups', {
      ...
      ui.toast(ui.Icons.ListAlt, 'Preset shared', function()
        removeSetup(response.setupID)
      end)
      listOfSetups = nil
    end, function(err)
      ui.toast(ui.Icons.Warning, 'Failed to share preset: ' .. err)
    end)
  end, function(err)
    ui.toast(ui.Icons.Warning, 'Failed to share preset: ' .. err)
  end)
...
local function commentsBlock()
  ...
  if (ui.button('Send', vec2(ui.availableSpaceX(), 0), canSend and 0 or ui.ButtonFlags.Disabled) or submitted) and canSend then
    ...
    rest('POST', 'comments', { setupID = item.setupID, userName = AppConfig.userName, data = comment:trim() },
      function(response)
...
local function windowGeneric()
  ...
  if authorUsernameFilter ~= '' then
    ...
    ui.sameLine()
    local n = string.format('%d preset%s by %s:', setupsTotalCount, setupsTotalCount == 1 and '' or 's',
      authorUsernameFilter)
  ...
  elseif searchFilter ~= '' then
    ...
    ui.addIcon(ui.Icons.ArrowLeft, iconSize, iconAlign, rgbm.colors.white)
    ui.sameLine()
    ui.header(string.format('%d preset%s matching “%s”:', setupsTotalCount, setupsTotalCount == 1 and '' or 's',
      searchFilter))
  else
    ui.header(string.format('%d preset%s', setupsTotalCount, setupsTotalCount == 1 and '' or 's'))
  end
  ...
  ui.combo('##sort', setupsOrder[AppConfig.setupsOrder][1], ui.ComboFlags.HeightLarge, function()
    ...
    ui.offsetCursorY(12)
    ui.header('Search:')
    local updated, _, enter = ui.inputText('Name or author', searchFilter,
  ...
  ui.childWindow('scroll', ui.availableSpace():sub(vec2(0, 40)), function()
    ui.offsetCursorY(8)
    if #setups == 0 then
      ui.textDisabled('No presets shared yet.')
      return
    end
    ...
    for i = f, math.min(t, #setups) do
      ...
      if v then
        ...
        if v.trackID == mainTrackID and DataProvider.targetName then
          if ui.button('     Apply', not currentlyApplying and 0 or ui.ButtonFlags.Disabled) then
            currentlyApplying = true
            getSetupData(v, true, function(err, data)
              currentlyApplying = false
              if err then
                ui.toast(ui.Icons.Warning, 'Failed to load presets: ' .. err)
              else
                try(function()
                  local added, current = DataProvider.add(data), DataProvider.current()
                  ui.toast(ui.Icons.Confirm, 'Setup loaded', function()
                    ui.toast(ui.Icons.Confirm, 'Previous presets restored')
                    DataProvider.select(current)
                    DataProvider.delete(added)
                  end)
                  DataProvider.select(added)
                end, function(err)
                  ui.toast(ui.Icons.Warning, 'Failed to load presets: ' .. err)
                end)
              end
          ...
          if ui.itemHovered() then
            ...
            ui.setTooltip(
              'Preset will be instantly applied to “%s” (with an option to revert back). To download a preset, use context menu.' %
              DataProvider.targetName)
            ui.popStyleVar()
          end
          ui.pushStyleVar(ui.StyleVar.Alpha, 1)
          ...
          if ui.itemHovered() then
            ui.pushStyleVar(ui.StyleVar.Alpha, 1)
            ui.setTooltip(
              'These presets are for a different track, but you can download them and use them in the next race.')
            ui.popStyleVar()
          end
        end
  ...
  if ui.button('Share a preset', vec2(ui.availableSpaceX(), 0), sessionID ~= nil and 0 or ui.ButtonFlags.Disabled) then
    local shareable = table.filter(DataProvider.list(), function(item) return item.metadata.canBeShared end) ---@type StoreEntry[]
    local selected ---@type StoreEntry?
    local name
    ui.modalDialog('Share a preset', #shareable == 0 and function()
      ui.textWrapped('No presets to share. Create and save new presets using a built-in editor.')
      ui.newLine()
      ui.offsetCursorY(4)
      ...
      if selected then
        ...
        ui.text('Select a preset to share:')
...
function exchange.open(targetName, dstDir, callbacks)
  local listCache
  DataProvider = {
    targetName = targetName,
    list = function()
      if not listCache or listCache[1] ~= callbacks.list() then
        listCache = { callbacks.list(), table.map(callbacks.list(), function(item)
          ---@type StoreEntry
          return {
            filename = item.filename,
            metadata = { name = item.name, canBeShared = true },
            export = function(s, name)
              local e = stringify.tryParse(io.load(item.filename))
              if type(e) ~= 'table' then
                error('Preset is damaged')
              end
              return stringify({ data = e, name = name }, true)
            end
          }
        end) }
      end
    ...
    add = function(data, trackID)
      local parsed = stringify.tryParse(data)
      if type(parsed) ~= 'table' or not parsed.name or not parsed.data or not io.isFileNameAcceptable(parsed.name) then
        error('Preset is damaged')
      end
      io.createDir(dstDir)
      io.save(dstDir .. '/' .. parsed.name .. '.lon', stringify(parsed.data), true)
      return '/' .. parsed.name
    end,
    current = function()
    ...
    select = function(id)
      if id:sub(1, 1) == '/' then
        callbacks.load(io.load(dstDir .. id .. '.lon'), io.getFileName(id:sub(2)))
      else
        callbacks.load(id)
      end
    end,
    delete = function(id)
      if id:sub(1, 1) == '/' then
        io.deleteFile(dstDir .. id .. '.lon')
      end
    end
  }
  ui.popup(function()
    if not callbacks.valid() then
      ui.closePopup()
    end
    windowGeneric()
  end, {
    title = 'RGB Exchange',
    size = { min = vec2(200, 240), initial = vec2(320, 400) },
  })
end
