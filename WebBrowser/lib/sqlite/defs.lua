...
--   if statement == conn_ptr then
--     -- CSP: simplify subsequent transactions
--     return fn()
--   end
--   M.exec_stmt(conn_ptr, "BEGIN")
--   statement = conn_ptr
--   local res, ret = pcall(fn) -- CSP: safe call
--   statement = nil
--   if res then
--     M.exec_stmt(conn_ptr, "COMMIT")
--     return ret
--   else
--     M.exec_stmt(conn_ptr, "ROLLBACK") -- CSP: with a rollback if needed
--     error(ret, 2)
--   end
-- end

-- Let’s instead get some speed and make things really convenient:
...
M.wrap_stmts = function(conn_ptr, fn)
  if not statement then
    ...
    statement = conn_ptr
    setTimeout(function ()
      M.exec_stmt(statement, "COMMIT")
      statement = nil
    end)
  elseif statement ~= conn_ptr then
...
M.connect = function(uri, opts)
  ...
  if code ~= M.flags.ok then
    error(("sqlite.lua: couldn't connect to sql database, error code: %s"):format(code))
  end
