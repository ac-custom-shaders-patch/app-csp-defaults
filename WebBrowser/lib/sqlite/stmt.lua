...
function sqlstmt:val(idx)
  ...
  elseif ktype == 4 then
    -- CSP: binary blobs
    local ptr = clib.column_blob(self.pstmt, idx)
    if ptr == nil then return nil end
    return stringify.binary.parse(ptr)
  elseif ktype == 3 then
    -- CSP: use size in bytes
    local ptr = clib.column_text(self.pstmt, idx)
    if ptr == nil then return nil end
    local size = clib.column_bytes(self.pstmt, idx)
    return ffi.string(ptr, size)
  elseif ktype == 1 then
    -- CSP: might be faster this way
    return clib.column_int(self.pstmt, idx)
  elseif ktype == 2 then
    return clib.column_double(self.pstmt, idx)
  end
  return nil
end
...
function sqlstmt:bind(...)
  ...
  if type(args[1]) == "number" and args[2] then
    ...
    if type(value) == 'table' then
      ...
      -- local i = stringify(value)
      return clib.bind_blob64(self.pstmt, idx, i, #i, nil)
    elseif type(value) == 'string' then
      return clib.bind_text(self.pstmt, idx, value, #value, nil)
    elseif type(value) == 'number' then
      return clib.bind_double(self.pstmt, idx, value)
    else
      if value ~= nil then error('Not supported: '..type(value)) end
      return clib.bind_null(self.pstmt, idx, value)
    end
  end
end
